      ...
      <div className='flex flex-col gap-3'>
       {dummyConnectionsData.map((user) =>(
        ...
        <div onClick={()=> navigate (`/messages/${user._id}`)} className='flex flex-col gap-2 mt-4'>
          ...
          bg-slate-100 hover:bg-slate-200 text-slate-800 activate:scale-95 transition cursor-pointer gap-1'>
            <MessageSquare className="w-4 h-4"/>
          </button>

          <button onClick={()=> navigate (`/profile/${user._id}`)} className='size-10 flex items-center justify-center text-sm rounded
          bg-slate-100 hover:bg-slate-200 text-slate-800 activate:scale-95 transition cursor-pointer'>
