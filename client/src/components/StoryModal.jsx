...
const StoryModal = ({setShowModal, fetchStories}) => {
            ...
            if(file.type.startsWith("video")){
                ...
                video.onloadedmetadata = ()=>{
                    ...
                    if(video.duration > MAX_VID_DUR){
                        toast.error("Duration cannot exceed 1 minute")
                        setMedia(null)
                        setPreviewUrl(null)
                    }else{
                        setMedia(file)
    ...
    const handleCreateStory = async () => {
        ...
        if(media_type === "text" && !text) throw new Error ("Enter some text")
  ...
  return (
    <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
        <div className='w-full max-w-md'>
            ...
            <button onClick={()=> toast.promise(handleCreateStory(), {loading: 'Saving...'})} className='flex items-center justify-center gap-2 text-white py-3 mt-4 w-full rounded bg-gradient-to-r from-red-400 to-rose-600 hover:from-red-600 hover:to-rose-700 active:scale-95 transition cursor-pointer'>
